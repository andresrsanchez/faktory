...
// 			data := fakeJob()
// 			jid1 := data.Jid
// 			err = retries.AddElement(util.Nows(), data)
// 			assert.NoError(t, err)
...
// 			err = q.AddElement(ts, data)
// 			assert.NoError(t, err)

// 			req, err := ui.NewRequest("GET", fmt.Sprintf("http://localhost:7420/retries/%s|%s", ts, data.Jid), nil)
// 			assert.NoError(t, err)
// 			w := httptest.NewRecorder()
// 			retryHandler(w, req)
// 			assert.Equal(t, 200, w.Code)
// 			assert.True(t, strings.Contains(w.Body.String(), data.Jid), w.Body.String())
// 		})
...
// 			err = q.AddElement(util.Nows(), data)
// 			assert.NoError(t, err)
...
// 			req, err := ui.NewRequest("GET", fmt.Sprintf("http://localhost:7420/scheduled/%s|%s", ts, data.Jid), nil)
// 			assert.NoError(t, err)
// 			w := httptest.NewRecorder()
...
// 			req, err := ui.NewRequest("GET", fmt.Sprintf("http://localhost:7420/morgue/%s|%s", ts, data.Jid), nil)
// 			assert.NoError(t, err)
// 			w := httptest.NewRecorder()
// 			deadHandler(w, req)
// 			assert.Equal(t, 200, w.Code)
